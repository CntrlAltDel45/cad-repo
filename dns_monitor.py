...
def check_dns():
    try:
        ...
        if current_ip is None:
            ...
            # IP has changed
            duration_since_last_change = now - datetime.strptime(
                os.path.getmtime(current_ip_file), '%Y-%m-%d %H:%M:%S')
            with open(current_ip_file, "w") as f:
                f.write(resolved_ip)
            log_message(
                f"IP change detected: {current_ip} -> {resolved_ip} at {now.strftime('%Y-%m-%d %H:%M:%S')}\n"
                f"Duration with previous IP ({current_ip}): {str(duration_since_last_change).split('.')[0]}"
            )
